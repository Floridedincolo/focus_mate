#!/bin/bash
# Script to fix the block_app plugin compilation error
# Run this if you encounter "Unresolved reference 'ServiceInfo'" error again

echo "Fixing block_app plugin..."

PLUGIN_PATH="$HOME/.pub-cache/hosted/pub.dev/block_app-0.0.1/android/src/main/kotlin/com/block_app/AppBlockingService.kt"

if [ ! -f "$PLUGIN_PATH" ]; then
    echo "Error: block_app plugin not found. Run 'flutter pub get' first."
    exit 1
fi

cat > "$PLUGIN_PATH" << 'EOFFILE'
...
echo "✅ block_app plugin fixed successfully!"
