...
object AppManager : MethodCallHandler {
    ...
    fun init(activity: Activity) {
        ...
        android.util.Log.d("BLOCK_DEBUG", "AppManager.init called; blockedApps=${blockedApps}")
    }
    ...
    private fun startBlockingService() {
        activity?.let {
            try {
                // Use explicit Intent with context and service class to avoid component resolution issues
                val serviceIntent = Intent(it, AppBlockingService::class.java)

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    androidx.core.content.ContextCompat.startForegroundService(it, serviceIntent)
                    android.util.Log.d("BLOCK_DEBUG", "startBlockingService: startForegroundService called (component=${it.packageName}/${AppBlockingService::class.java.name})")
                } else {
                    it.startService(serviceIntent)
                    android.util.Log.d("BLOCK_DEBUG", "startBlockingService: startService called (component=${it.packageName}/${AppBlockingService::class.java.name})")
                }
            } catch (e: Exception) {
                android.util.Log.e("BLOCK_DEBUG", "startBlockingService failed", e)
            }
        } ?: android.util.Log.w("BLOCK_DEBUG", "startBlockingService: activity is null, cannot start service")
    }
    ...
    private fun stopBlockingService() {
        activity?.let {
            try {
                ...
                android.util.Log.d("BLOCK_DEBUG", "stopBlockingService: stopService called (component=${it.packageName}/${AppBlockingService::class.java.name})")
            } catch (e: Exception) {
                android.util.Log.e("BLOCK_DEBUG", "stopBlockingService failed", e)
            }
        } ?: android.util.Log.w("BLOCK_DEBUG", "stopBlockingService: activity is null, cannot stop service")
    }
    ...
    fun loadBlockedApps(context: Context) {
        ...
        android.util.Log.d("BLOCK_DEBUG", "loadBlockedApps: loaded=${blockedApps}")
    }
    ...
    private fun saveBlockedApps(context: Context? = null) {
        ...
        android.util.Log.d("BLOCK_DEBUG", "saveBlockedApps: saved=${blockedApps}")
    }
